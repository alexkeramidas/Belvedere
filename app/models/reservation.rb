class Reservation < ActiveRecord::Base
    ...
    validates_format_of :adults, :with => /\A^(1[0-9]|[1-9])$\Z/i, :message => 'Number of adults must be between 1 and 19'
    validates_format_of :youngsters, :with => /\A^([0-1][0-9]|[0-9])$\Z/i, :message => 'Number of children must be between 0 and 19'
    validates :arrival, :date => {:after => Proc.new { Time.now }, :before => Proc.new { Time.now + 6.month } , :message => 'Arrival date must be after the current date'}
    validates :departure, :date => {:after => :arrival , :before => Proc.new { Time.now + 1.day + 6.month }, :message => 'Departure date must be after the arrival date and less than six months in the future'}
